                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                    ...
                    @if ($transaksi->pembayaran == 'dp')
                        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                            ...
                            <p class="mt-1 text-lg font-bold text-amber-700">Rp {{ number_format($transaksi->jumlah_dp, 0, ',', '.') }}</p>
                        </div>
                        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Sisa</p>
                            <p class="mt-1 text-lg font-bold text-red-600">Rp {{ number_format($transaksi->total - $transaksi->jumlah_dp, 0, ',', '.') }}</p>
                        </div>
                    @endif
                    ...
                    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <p class="mt-1 text-xl font-bold text-gray-800">Rp {{ number_format($transaksi->total, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Layanan</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Harga Satuan</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Berat</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Subtotal</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($transaksi->details as $item)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-5 py-4 text-gray-700">Rp {{ number_format($item->paket->harga, 0, ',', '.') }}</td>
                                        <td class="px-5 py-4 text-gray-700">{{ $item->berat }} {{ $item->paket->satuan }}</td>
                                        <td class="px-5 py-4 font-semibold text-gray-800">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="mt-6 bg-linear-to-r from-gray-50 to-gray-100 rounded-xl p-5 border border-gray-200 max-w-md ml-auto">
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">Rp {{ number_format($transaksi->details->sum('subtotal'), 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                ...
                <div class="flex flex-wrap gap-3 items-center justify-end">
                    ...
                    <form action="{{ route('transaksi.update-status', $transaksi->id) }}" method="POST" class="inline-block">
                        ...
                        <select name="status_order"
                            ...
                            <option value="baru" {{ $transaksi->status_order == 'baru' ? 'selected' : '' }}>Status: Baru</option>
                            <option value="diproses" {{ $transaksi->status_order == 'diproses' ? 'selected' : '' }}>Status: Diproses</option>
                            <option value="selesai" {{ $transaksi->status_order == 'selesai' ? 'selected' : '' }}>Status: Selesai</option>
                            <option value="diambil" {{ $transaksi->status_order == 'diambil' ? 'selected' : '' }}>Status: Diambil</option>
                        </select>
                    </form>
                    ...
                    <form action="{{ route('transaksi.update-pembayaran', $transaksi->id) }}" method="POST" class="inline-block">
                        ...
                        <select name="pembayaran"
                            ...
                            onchange="this.form.submit()">
                            <option value="dp" {{ $transaksi->pembayaran == 'dp' ? 'selected' : '' }}>Pembayaran: DP</option>
                            <option value="lunas" {{ $transaksi->pembayaran == 'lunas' ? 'selected' : '' }}>Pembayaran: Lunas</option>
                        </select>
                    </form>

                    <!-- Print Button -->
                    <a href="{{ route('export.invoice.pdf', $transaksi->id) }}"
                        class="inline-flex items-center gap-2 rounded-lg bg-linear-to-r from-emerald-500 to-teal-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md hover:from-emerald-600 hover:to-teal-700 transition-all hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-emerald-400">
